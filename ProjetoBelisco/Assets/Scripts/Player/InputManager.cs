 ...
 */
     ...
     */
        ...
        [BoxGroup("Player Info")] [ShowInInspector] private Rewired.Player _playerController;
     ...
     */
        ...
        private Grounder _grounder = null;
        private ComboManager _attack = null;
        private PlatformDown _platformDown = null;
     ...
     */
        ...
        {
            _grounder.OnGrounded += ListenGrounder;
        }
     ...
     */
        private void GetEssentialComponentsComponent()
        {
            ...
            _grounder = this.GetComponent<Grounder>();
            _jump = this.GetComponent<Jump>();
            _attack = this.GetComponent<ComboManager>();
            _platformDown = this.GetComponent<PlatformDown>();
        }
            ...
            if (!IsControllerLocked() && !IsAttackLocked())
            {
                ...
                if(!gotDownFromPlatform)
                {
                    ...
                    if (((_curentInputs & Inputs.JumpFollowUp) == Inputs.JumpFollowUp))
                    {
                        if (_jumpCicle)
                        {
                            if (_jump.CanRaiseJump(_jumpActionTimestamp))
                            {
                                this.Jump();
                            }
                        }
     ...
     */
        private void Move()
        {
            _movement.MovePlayer(MoveDirection.x, _movementAxisTimeCounter);
        }
     ...
     */
        ...
        {
            _movement.StopMovement(_movementAxisTimeCounter);
        }
     ...
     */
        ...
        private void Attack()
        {
            _attack.AttackCommand(CalculateDirections(MoveDirection));
        }

        private bool GetDownFromPlatform()
        {
            return _platformDown.GetDownFromPlatform();
        }
